...
const els = {
  url: $("url"), fetch: $("fetch"), fetchError: $("fetch-error"),
  info: $("info"), thumb: $("thumb"), title: $("title"), subtitle: $("subtitle"),
  quality: $("quality"), mode: $("mode"), outdir: $("outdir"),
  chooseDir: $("choose-dir"), filename: $("filename"), start: $("start"),
...
function renderInfo(info) {
  ...
  if (info.durationSeconds != null) bits.push(formatDuration(info.durationSeconds));
  ...
  for (const h of info.heights) els.quality.add(new Option(`${h}p`, `${h}p`));
...
grapplehook.onProgress((p) => {
  ...
  if (!v || v.root.classList.contains("settled")) return;
  ...
  if (p.speed != null) bits.push(`${formatBytes(p.speed)}/s`);
  if (p.eta != null) bits.push(`ETA ${formatDuration(p.eta)}`);
